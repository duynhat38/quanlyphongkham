<template>
    <div>
        <vue-bootstrap4-table id="table" :rows="rows" :columns="columns" :config="config" :actions="actions"
            ...
            <template slot="prescription" slot-scope="props">
                <span><i class="bi bi-clipboard-plus" @click="keDonThuoc(props.row)"
                        v-b-modal="'KeDonThuocModal'"></i></span>
            </template>
        ...
        <ke-don-thuoc :booking="bookingKeDonThuoc" @editMode="handleEditMode">
        </ke-don-thuoc>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        keDonThuoc(booking) {
            this.bookingKeDonThuoc = booking;
        },
