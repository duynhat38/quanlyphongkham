  ...
  <div>
    <vue-bootstrap4-table id="table" :rows="rows" :columns="columns" :config="config" :actions="actions"
      ...
      <template slot="change" slot-scope="props">
        <span><i class="bi bi-card-checklist" @click="editBenhNhan(props.row)"
            v-b-modal="'EditBenhNhanModal'"></i></span>
      </template>
      <template slot="remove" slot-scope="props">
        <span><i class="bi bi-shield-x" @click="removeBenhNhan(props.row)"></i></span>
      </template>
    </vue-bootstrap4-table>
    <edit-benh-nhan :patient="patientEdit" @editMode="handleEditMode"></edit-benh-nhan>
  </div>
</template>
<script>
import VueBootstrap4Table from "vue-bootstrap4-table";
import PatientsService from "@/services/PatientsService";
import EditBenhNhan from "@/components/EditBenhNhan.vue";
...
export default {
  ...
  data() {
    return {
      ...
      actions: [
        {
          btn_text: "Thêm Bệnh Nhân",
          event_name: "addBenhNhan",
      ...
      config: {
        ...
        card_title: "Quản Lý Hồ Sơ Bệnh Nhân",
  ...
  async created() {
    this.rows = await this.getAllBenhNhan();
  },
  watch: {},
  methods: {
    ...
    addBenhNhan() {
      ...
      this.$bvModal.show("EditBenhNhanModal");
    },
    async editBenhNhan(payload) {
      const patient = await this.getDataBenhNhan(payload._id);
      if (patient) {
        this.patientEdit = patient;
      }
    },
    removeBenhNhan(patient) {
      this.$swal({
        ...
        if (result) {
          try {
            const del = await PatientsService.delete({
    ...
    async getAllBenhNhan() {
      try {
        const response = await PatientsService.all();
        if (response) {
    ...
    async getDataBenhNhan(patientId) {
      try {
        const res = await PatientsService.getPatient(patientId);
        if (res.data.success) {
