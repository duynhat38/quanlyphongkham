  ...
  <div>
    <vue-bootstrap4-table id="table" :rows="rows" :columns="columns" :config="config" :actions="actions"
      ...
      <template slot="change" slot-scope="props">
        <span><i class="bi bi-card-checklist" @click="editBooking(props.row)" v-b-modal="'EditBookingModal'"></i></span>
      </template>
      ...
      <template slot="status" slot-scope="props">
        <b>{{ changeStatus(props.cell_value) }}</b>
      </template>
    ...
    <phieu-kham-chua-benh :booking="bookingHealthCare" :services="services" @editMode="handleEditMode">
    </phieu-kham-chua-benh>
  </div>
</template>
<script>
...
export default {
  ...
  async created() {
    this.clinic = await this.findClinicByReceptionist(this.$store.state.user._id);
    if (!this.clinic) {
      return this.alertDisplay('Thông Báo', 'Bạn chưa được phân nhiệm vụ trong phòng khám nào !', 'info');
    }
    this.rows = await this.getAllBooking(this.clinic._id);
  ...
  methods: {
    ...
    async getAllBooking(clinicId,date) {
      try {
        const response = await BookingsService.all({
          clinicId: clinicId,
          date: date
        });
        if (response) {
    ...
    async findClinicByReceptionist(receptionistId) {
      try {
        const res = await ClinicsService.findClinicByReceptionist({
