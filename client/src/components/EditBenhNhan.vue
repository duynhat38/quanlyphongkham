  ...
  <b-modal id="EditBenhNhanModal" title="Thông tin bệnh nhân" @ok="handleOk" ok-title="Cập nhật" cancel-title="Thoát"
    ...
    <div>
      ...
      <div class="col-sm-12" v-if="!isAddBenhNhan">
        <h6>Lịch sử khám chữa bệnh <span class="badge badge-success badge-pill">{{ prescriptions.length }} </span></h6>
        <code v-if="prescriptions.length <= 0">Bệnh nhân chưa khám lần nào !</code>
        <table class="table table-hover" v-else>
          ...
          <tbody>
            <tr v-for="(prescription, index) in prescriptions" :key="index">
              <td>{{ moment(prescription.createdAt).format('MM/DD/YYYY') }}</td>
...
export default {
  ...
  watch: {
    patient: async function (patient) {
      if (!patient) {
        this.fullname = null;
        this.phone = null;
        this.address = null;
        ...
        this.prescriptions = await this.getDataLichSuKham(patient._id);
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    getDataLichSuKham: async function (patientId) {
      try {
        const response = await PrescriptionsService.getPrescriptionsOfPatient({
          patientId: patientId
        });
